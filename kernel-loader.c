...
getProcStatField(enum Procstat_t type, char *out, size_t len)
{
  ...
  fd = __real_open(procPath, O_RDONLY);

  num_read = __real_read(fd, sbuf, sizeof sbuf - 1);
  __real_close(fd);
...
{
  Area area;
  int mapsfd = __real_open("/proc/self/maps", O_RDONLY);
...
createNewStackForRtld(const DynObjInfo_t *info)
{
  ...
  void *newStack = __real_mmap(NULL, stack.size, PROT_READ | PROT_WRITE,
...
{
  void *addr = __real_mmap(0, PAGE_SIZE, PROT_READ | PROT_WRITE,
