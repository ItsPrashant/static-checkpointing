...
safeLoadLib(const char *name)
{
  ...
  int cmd_fd = __real_open(name, O_RDONLY);
  ...
  ld_so_fd = __real_open(elf_interpreter, O_RDONLY);
                    ...
                    char* elf_interpreter, void *ld_so_addr)
{
  ...
  rc = __real_read(fd, e_ident, sizeof(e_ident));
  ...
  Elf64_Ehdr elf_hdr;
  rc = __real_read(fd, &elf_hdr, sizeof(elf_hdr));
  ...
  *cmd_entry = elf_hdr.e_entry;
  ...
  __real_lseek(fd, phoff, SEEK_SET);
  for (i = 0; i < elf_hdr.e_phnum; i++) {
    assert(i < elf_hdr.e_phnum);
    rc = __real_read(fd, &phdr, sizeof(phdr)); // Read consecutive program headers
#ifdef UBUNTU
    if (phdr.p_type == PT_INTERP) break;
  }
  __real_lseek(fd, phdr.p_offset, SEEK_SET); // Point to beginning of elf interpreter
  assert(phdr.p_filesz < MAX_ELF_INTERP_SZ);
  rc = __real_read(fd, elf_interpreter, phdr.p_filesz);
...
get_symbol_offset(const char *libname, const char *symbol)
{
  ...
  __real_lseek(fd, symtab.sh_offset, SEEK_SET);
  for ( ; __real_lseek(fd, 0, SEEK_CUR) - symtab.sh_offset < symtab.sh_size; ) {
    rc = __real_read(fd, &symtab_entry, sizeof symtab_entry);
...
map_elf_interpreter_load_segment(int fd, Elf64_Phdr phdr, void *ld_so_addr)
{
  ...
  rc2 = __real_mmap((void *)addr, size, prot, flags, fd, offset);
  if (rc2 == MAP_FAILED) {
  ...
  if (endBss > startBss) {
    ...
    rc2 = __real_mmap(base, len, prot, flags, -1, 0);	//ERROR: rc2 gets modified here?
    if (rc2 == MAP_FAILED) {
      DLOG(ERROR, "Failed to map memory region at %p. Error:%s\n",
...
getSymbolTable(const char *libname, Elf64_Shdr *symtab, char **strtab)
{
  ...
  while (retries < 2) {
    ...
    __real_lseek(fd, shoff, SEEK_SET);
    for (int i = 0; i < elf_hdr.e_shnum; i++) {
      rc = __real_read(fd, &sect_hdr, sizeof sect_hdr);
...
{
  off_t currOff = __real_lseek(fd, 0, SEEK_CUR);
  off_t sidx_off = ehdr->e_shentsize * sidx + ehdr->e_shoff;
  __real_lseek(fd, sidx_off, SEEK_SET);
  int rc = __real_read(fd, shdr, sizeof *shdr);
  assert(rc == sizeof *shdr);
  rc = __real_lseek(fd, shdr->sh_offset, SEEK_SET);
  if (rc > 0) {
    *data = __real_malloc(shdr->sh_size);
    rc = __real_lseek(fd, shdr->sh_offset, SEEK_SET);
    rc = __real_read(fd, *data, shdr->sh_size);
    assert(rc == shdr->sh_size);
  }
  __real_lseek(fd, currOff, SEEK_SET);
